...
const ValidarFormularioDatos=(orden)=>{
    switch(orden.target.name){
        case "FechaCreacionName":
            ...
            // Si no cumple la funcion da error
            if (NombreTrabajador===""){
                ...
                estadoCampos.TecnicoName = false;
                console.log("mal");}
            else if (NombreTrabajador === "" || NombreTrabajador.length < 3) {  
                mostrarError(orden.target, "El nombre del tecnico no puede estar vacio y  tener mínimo 3 caracteres");
                estadoCampos.TecnicoName= false;
                console.log("mal")

            }
            // Si  cumple la funcion se va el mensaje
            else {
                estaBien(orden.target);
...
// codigo tomamos id del formulario de datos de la orden  y se realiza eveneto, si sale del input
// o levanta tecla
...
// misma logica y mismas validaciones aplicadas al formualrio de datos del carro 
...
// fin de las validaciones del formulario  carro 
...
// fin de las validaciones de los campo de descripcion 1 
...
// Función para verificar si todos los campos están validados correctamente
